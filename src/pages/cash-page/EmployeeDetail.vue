...
export default {
  ...
  setup(props, context) {
    const refFirstTabindex = ref(null);
    const { state, commit } = useStore();
    const { employeeModal, modalMode } = toRefs(props);
    console.log(employeeModal.value);
    ...
    const saveDataAndClose = async () => {
      try {
        ...
        if (!isResponseSuccess) {
          ...
          getErrorAfterExcuteApi(responseError);
          notifyError();
          // showToastMessage(ToastMessage.ERROR);
          return;
        }
    ...
    const saveData = async () => {
      try {
        ...
        if (!isResponseSuccess) {
          const responseError = response.Data;
          const apiErrors = getErrorAfterExcuteApi(responseError);
          notifyError(apiErrors);
          // showToastMessage(ToastMessage.ERROR);
          return;
        }
        showToastMessage(ToastMessage.SUCCESS);
    ...
    const excuteApi = async () => {
      ...
      switch (modalMode.value) {
        case FormEnum.MODE_EDIT:
          result = await updateEmployee(jsonEmployee.EmployeeID, jsonEmployee);
          response = {
     ...
     * NXTSAN 18-09-2022
     */
    const getErrorAfterExcuteApi = (responseError) => {
      try {
    ...
    const handleFocusFirst = () => {
      refFirstTabindex.value.$el.children[0].focus();
    };
